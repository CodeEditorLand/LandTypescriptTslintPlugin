...
export class TsLintConfigurationStatusBarWarning {
	...
	public constructor(
		...
		this._disposables.push(
			vscode.commands.registerCommand(this.showHelpCommand, async () => {
				const manageTrust: vscode.MessageItem = {
					title: "Manage Library Execution",
					isCloseAffordance: true,
				};

				const help: vscode.MessageItem = {
					title: "Help",
					isCloseAffordance: true,
				};
				...
				const result = await vscode.window.showErrorMessage(
					this._statusBarItem.tooltip || "TSLint Error",
					manageTrust,
					help,
					close
				);
	...
	private _updateForActiveEditor(
		...
		const notUsingWorkspaceVersionError = diagnostics.find(
			(diagnostic) =>
				diagnostic.source === "tslint" &&
				diagnostic.message.startsWith(
