...
export class WorkspaceLibraryExecutionManager {
	...
	constructor(context: {
		...
		this._disposables.push(
			vscode.commands.registerCommand(manageWorkspaceTrustCommand, () => {
				this.showTrustUi();
			}),
		);

		this._disposables.push(
			vscode.commands.registerCommand(resetWorkspaceTrustCommand, () => {
				this.reset();
			}),
		);
	}
	...
	public async showTrustUi(): Promise<void> {
		const currentIsTrustedWorkspaceValue =
			this.workspaceMemento.get<boolean>(this.isTrustedWorkspaceKey);

		const trustItem: vscode.QuickPickItem = {
			label: "Enable workspace library execution",
			description:
				currentIsTrustedWorkspaceValue === true
					? "(currently active)"
					: undefined,
		...
		const untrustItem: vscode.QuickPickItem = {
			...
			description:
				currentIsTrustedWorkspaceValue === false
					? "(currently active)"
					: undefined,
		...
		const trustGloballyItem: vscode.QuickPickItem = {
			...
			description: this.globalMemento.get(this.isTrustedGloballyKey)
				? "(enabled)"
				: undefined,
		...
		const result = await vscode.window.showQuickPick(
			[trustItem, untrustItem, trustGloballyItem, helpItem],
			{
